...
public class SearchCountryByCriteriaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        countrySearchCriteria.setName(request.getParameter("name"));
        countrySearchCriteria.setContinent(request.getParameter("continent"));
        ...
        countrySearchCriteria.setPopulation(population);

        String pageParam = request.getParameter("page");
        Integer page = pageParam != null ? Integer.valueOf(pageParam) : 1;
        countrySearchCriteria.setPage(page);

        String limitParam = request.getParameter("limit");
        Integer limit = limitParam != null ? Integer.valueOf(limitParam) : LIMIT;
        countrySearchCriteria.setLimit(limit);

        String acceptType = request.getHeaders("Accept").nextElement();

        logger.info("Getting country with parameters: {}", countrySearchCriteria);

        if (acceptType.contains(MimeType.APPLICATION_XML.getValue())) {
            List<Country> country = countryService.getCountriesByCriteria(countrySearchCriteria);
